	...
	return this.each(function(){
		...
		*/
		...
		if (!settings.autoSize) {
		    pictWidth = settings.picWidth;
		    pictHeight = settings.picHeight;
		    container.find("img").width(pictWidth);
		    container.find("img").height(pictHeight);
		}
		var pictEls = container.find("li").size();
		//alert(pictWidth + " " + pictEls + " " + settings.thumbs);
		...
		container.each(function(i) {
			if(settings.typo)
			{
			    jQuery(this).find("img").each(function (z) {
			        var svwpImg = jQuery(this);
			        svwpImg.addClass("svwpBImg");
			        if (settings.clickfunc)
			        {
			            svwpImg.click(settings.clickfunc);
			        }
			            svwpImg.after("<span class='typo' style='position: absolute; width:" + (pictWidth - 12) + "px; margin: 0 0 0 -" + pictWidth + "px'>" + jQuery(this).attr("alt") + "<\/span>");
			});
			}
			...
			jQuery(this).find("li").each(function (n) {
			    jQuery("div#thumbSlider" + j + " ul").append("<li><a title='" + jQuery(this).find("img").attr("alt") + "' href='#'><img width='" + thumbsWidth + "' height='" + thumbsHeight + "' onerror=\"function(img){$.imgOnError(img);}\" src='" + jQuery(this).find("img").attr("src") + "' class='svwpSImg' /><p class='tmbrdr'>&nbsp;<\/p><\/a><\/li>");
			});
			...
			jQuery("div#svwp"+ j + ", div#ui" + j).wrapAll("<div style='width:"+ pictWidth +"px; margin:0 auto;'><\/div>");
			});
