...
(function ($) {
    ...
    function init(target) {
        ...
        function createMenu(menu) {
            ...
            if (menu.hasClass('menu-content')) {
                ...
                menu.children('div').each(function () {
                    var item = $(this);
                    if (options.maxLines > 0 && items++ >= options.maxLines) {
                        items = 1;
                        itemsGroup = [];
                        itemsGroups.push(itemsGroup);
                    }
                ...
                for (var i = 0; i < itemsGroups.length; i++) {
                    var ctl = $("<div class='menu-block'></div>").appendTo(menu);
                    if (i == itemsGroups.length - 1) {
                        ctl.addClass("last");
                    }
                    var line = $('<div class="menu-line"></div>').prependTo(ctl);
                    for (var j = 0; j < itemsGroups[i].length; j++) {
                        itemsGroups[i][j].appendTo(ctl);
                    }
                }
            }
    ...
    function setMenuWidth(target, menu) {
        ...
        menu.find('div.menu-text').each(function () {
            if (opts.maxLines > 0 && ++items >= opts.maxLines) {
                items = 0;
                columns++;
            }
        ...
        var pos = 0;
        menu.find('div.menu-line').each(function () {
            var offset = $(this).offset();
            offset.left = pos + offset.left;
            $(this).offset(offset);
            $(this)._outerHeight($(this).parent()._outerHeight()+4);
            pos += width;
        });
        width = width * columns + 16;
    ...
    function bindMenuItemEvent(target, item) {
        ...
        item.bind('click.menu', function () {
            ...
            // only the sub menu clicked and manualHide not set can hide all menus
            if (!this.submenu && !options.manualHide) {
                hideAll(target);
            ...
            options.onClick.call(target, item);
